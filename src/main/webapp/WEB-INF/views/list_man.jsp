					...
					<c:otherwise>
						...
						<c:forEach var="item" items="${output}" begin="0" end="9"
							...
							<li><a href="${viewUrl}"> <span class="thumb"><img
										src="${pageContext.request.contextPath}/assets/img/${item.origin_name}" alt="이미지1" /></span> <span
									class="text">${item.prod_name}</span> <span class="price">${item.prod_price}</span>
							</a></li>
						</c:forEach>
					</c:otherwise>
				</c:choose>
	...
	<div id="paging">
		...
		<c:choose>
			...
			<c:when test="${pageData.prevPage > 0}">
				<%-- 이동할 URL 생성 --%>
				<c:url value="list_man.do" var="prevPageUrl">
		...
		<c:forEach var="i" begin="${pageData.startPage}"
			...
			<%-- 이동할 URL 생성 --%>
			<c:url value="list_man.do" var="pageUrl">
		...
		<c:choose>
			...
			<c:when test="${pageData.nextPage > 0}">
				<%-- 이동할 URL 생성 --%>
				<c:url value="list_man.do" var="nextPageUrl">
