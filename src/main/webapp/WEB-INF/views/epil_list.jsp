...
<link href="${pageContext.request.contextPath}/assets/css/common.css"
	rel="stylesheet" />
<link
	href="${pageContext.request.contextPath}/assets/css/write_board.css"
	rel="stylesheet" />
...
<body>
				...
				<c:choose>
					...
					<c:otherwise>
						...
						<c:forEach var="item" items="${output2}" varStatus="status">
							...
							<%-- 상세페이지로 이동하기 위한 URL --%>
							<c:url value="/epil_detail.do" var="viewUrl">
								<c:param name="epil_bbs_no" value="${item.epil_bbs_no}" />
							</c:url>
	...
	<form action="${pageContext.request.contextPath}/epil_add.do">
	<input type="text" id="dw" name="dw" value="${output.prod_no}" style="display: none">
	<button type="submit" id="but1">글쓰기</button>
	</form>
</div>
		...
		<div id="fom">
			...
			<c:choose>
				...
				<c:when test="${pageData.prevPage > 0}">
					<%-- 이동할 URL 생성 --%>
					<c:url value="/epil_list.do" var="prevPageUrl">
			...
			<c:forEach var="i" begin="${pageData.startPage}"
				...
				<%-- 이동할 URL 생성 --%>
				<c:url value="/epil_list.do" var="pageUrl">
			...
			<c:choose>
				...
				<c:when test="${pageData.nextPage > 0}">
					<%-- 이동할 URL 생성 --%>
					<c:url value="/epil_list.do" var="nextPageUrl">
